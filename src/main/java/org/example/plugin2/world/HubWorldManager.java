package org.example.plugin2.world;
...
public class HubWorldManager {
    ...
    public HubWorldManager(Plugin2 plugin) {
        this.plugin = plugin;
    ...
    private void applyWorldRules() {
        ...
        hubWorld.setGameRule(GameRule.DO_DAYLIGHT_CYCLE, !heureFixe);
        if (heureFixe) {
            hubWorld.setTime(heureValeur);
        }

        hubWorld.setGameRule(GameRule.DO_WEATHER_CYCLE, !meteoBloquee);
        if (meteoBloquee) {
            hubWorld.setStorm(false);
            hubWorld.setThundering(false);
        }

        hubWorld.setGameRule(GameRule.DO_FIRE_TICK, !feuBloque);
        hubWorld.setGameRule(GameRule.DO_MOB_SPAWNING, mobSpawn);
