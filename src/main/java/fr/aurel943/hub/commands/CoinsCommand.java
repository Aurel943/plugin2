...
public class CoinsCommand implements CommandExecutor {
    ...
    public CoinsCommand(Hub plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!isKnownSubCommand(sousCommande)) {
            if (!sender.hasPermission("hub.coins.view-others")) {
    ...
    private void handleAdminOperation(CommandSender sender, String[] args, String action) {
        String permission = switch (action) {
            case "give" -> "hub.coins.give";
            case "take" -> "hub.coins.take";
            case "set" -> "hub.coins.set";
            default -> "hub.coins.give"; // ne devrait jamais arriver, sécurité par défaut
        };
    ...
    private void handleReset(CommandSender sender) {
        if (!sender.hasPermission("hub.coins.reset")) {
            messages.send(sender, "coins.permission-refusee-admin");
            return;
        }
    ...
    private void handleHelp(CommandSender sender) {
        if (!sender.hasPermission("hub.coins.help")) {
            messages.send(sender, "coins.permission-refusee-admin");
            return;
        }
